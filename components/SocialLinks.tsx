...
function openSocial(appUrl: string, webUrl: string) {
  ...
  setTimeout(() => {
    ...
    if (!leftPage && Date.now() - start < 2000) {
...
export function SocialLinks({
  ...
  return (
    <div className={variant === "button" ? "flex gap-4" : "flex gap-3"}>
      {SOCIALS.map((social) => (
        <a
          key={social.name}
          href={social.webUrl}
          ...
          className={
            variant === "button"
              ...
              : "flex h-8 w-8 items-center justify-center text-muted transition-colors hover:text-accent"
          }
        >
          <svg
            viewBox="0 0 24 24"
